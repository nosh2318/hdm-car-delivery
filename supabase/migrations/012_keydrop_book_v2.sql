-- ============================================================
-- 012 KEYDROP 予約RPC v2（埋め込み決済用・正確な金額計算）
-- 2026-06-10 / omni
-- 005 keydrop_book を拡張：
--  ・日数 = ceil(時間/24)（クライアントgetDayCountと一致。日付差ではなく実時間）
--  ・クラス料金（tier×日）＋オプション（補償・シート）をサーバ計算＝表示額と一致
--    補償: pack(安心パック)¥1650/日・cdw(免責)¥1100/日・none ¥0 ／ child¥1000・junior¥500
--  ・base_price=クラス計・option_price=オプション計・price=総額。時刻も保存。status=pending_payment
--  ・返却に total を含める→Edge Function(keydrop-pay)がこの額でSquare即時課金
-- ============================================================
...
begin
  ...
  -- 日数 = ceil(時間/24)・最低1
  v_days := greatest(1, ceil( extract(epoch from ((v_ret||' '||v_rett)::timestamp - (v_lend||' '||v_lendt)::timestamp)) / 86400.0 )::int);
