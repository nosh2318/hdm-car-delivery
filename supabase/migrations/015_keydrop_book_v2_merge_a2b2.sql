-- ============================================================
-- 015 KEYDROP 予約RPC v2：Aプラン→A/A2、Bプラン→B/B2 で割当（顧客は6プラン・在庫はフル活用）
-- 2026-06-11 / omni
-- 顧客サイトはA2/B2を非表示(6プラン)に統一。預かりのA2/B2車も「Aプラン/Bプラン」の予約で使えるよう、
-- 車両割当の type 条件を A→{A,A2} / B→{B,B2} に拡張。自社(A/B)を優先し、預かり(A2/B2)はオーバーフロー。
-- 変更点は割当クエリの type 条件 と order by のみ（013から）。reservation.vehicle は予約クラス(A/B)のまま。
-- ============================================================
...
begin
  ...
  -- Aプラン→A/A2、Bプラン→B/B2（預かり車も割当可能に）。それ以外は単一クラス。
  v_types := case v_class when 'A' then array['A','A2'] when 'B' then array['B','B2'] else array[v_class] end;
  ...
  -- 空き車両割当（A→A/A2・B→B/B2。自社クラス優先→預かりはオーバーフロー）
  select v.code, v.name into v_code, v_name from vehicles v
  where v.type = any(v_types) and coalesce(v.insurance_veh,false)=false and coalesce(v.active,true)=true
  ...
  order by (case when v.type=v_class then 0 else 1 end), v.code limit 1;
