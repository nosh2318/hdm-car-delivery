-- 035: SPK追跡RPCの目的地を「OPシート(tasks)最優先・reservationsは無視/最後の保険のみ」に。
-- オーナー方針: 場所の正はOPシートの場所欄(tasks DEL/COL・_ssPlace)。reservationsは信用しない。
-- NHAはnha_reservations側がOPシート(送迎場所)と一致するため従来通り。
...
language sql security definer set search_path = public as $$
  select r.kd_status,
    coalesce(_kd_task_place(r.id,'DEL'), nullif(r.del_place,'')) as del_place,
    coalesce(_kd_task_place(r.id,'COL'),
             (select nullif(t.col_place,'') from tasks t where t.reservation_id=r.id and coalesce(t.col_place,'')<>'' limit 1),
             nullif(r.col_place,'')) as col_place,
...
language sql security definer set search_path = public as $$
  select r.kd_status,
    coalesce(_kd_task_place(r.id,'COL'), _kd_task_place(r.id,'DEL'), nullif(r.del_place,'')) as del_place,
