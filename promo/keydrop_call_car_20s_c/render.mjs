...
const output = path.join(root, 'KEYDROP_車で遊ぼう_モーションエディトリアル_20sec.mp4');
const parts = [];
...
const photo = (name, image, duration, filter) => {
  const out = add(name);
  const base = "scale=1350:2400,zoompan=z='min(max(zoom,pzoom)+0.0022,1.20)':x='iw/2-(iw/zoom/2)+90-180*on/110':y='ih/2-(ih/zoom/2)+30*sin(on/15)':d=1:s=1080x1920:fps=30";
...
photo('02-road.mp4', '01-coast-drive.png', 3.2, [
  ...
  `drawtext=fontfile='${jp}':text='道が、遊び場になる。':fontsize=58:fontcolor=white:x=350:y=48`,
...
photo('05-hokkaido.mp4', '04-hokkaido-drive.png', 3.2, [
  ...
  `drawtext=fontfile='${jp}':text='次の景色へ。':fontsize=94:fontcolor=0x111111:x=68:y=225`,
].join(','));
