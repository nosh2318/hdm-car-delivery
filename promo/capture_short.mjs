...
const cast=process.argv[2]||'japanese', root=path.resolve(import.meta.dirname), frames=path.join(root,`frames_short_${cast}`);
...
await page.goto(`file://${path.join(root,'promo_short.html')}?cast=${cast}`);await page.evaluate(()=>window.ready);
const fps=24,duration=29;for(let i=0;i<fps*duration;i++){await page.evaluate(t=>window.renderAt(t),i/fps);await page.screenshot({path:path.join(frames,`frame_${String(i).padStart(5,'0')}.jpg`),type:'jpeg',quality:91});if(i%fps===0)process.stdout.write(`${cast}: ${i/fps}s / ${duration}s\n`)}
